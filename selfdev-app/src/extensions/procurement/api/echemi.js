...
export function echemiOperationLabel(operation) {
  const labels = {
    ...
    ECHEMI_INQUIRY_ALREADY_POSTED: 'Заявка по этому веществу уже размещена и видна всем продавцам Echemi. Вторая заявка не добавит адресатов — она покажет тот же запрос тем же людям ещё раз. Дождитесь предложений; если изменились объём или условия, снимите заявку на площадке и подготовьте новую.',
    ECHEMI_SUBMISSION_BLOCKED: 'Предыдущий клик площадка не подтвердила. Автоматический повтор запрещён: заявка могла уйти, и вторая попытка разместила бы её дважды. Закончите в браузере и запишите результат.',
    ECHEMI_APPROVAL_REQUIRES_PREVIEW: 'Сначала откройте и проверьте форму: согласовывать можно только то, что кто-то видел.',
