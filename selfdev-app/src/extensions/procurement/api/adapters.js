...
export function adaptSupplier(raw = {}) {
  return {
    ...
    businessRole: raw.businessRole ?? raw.business_role?.role ?? 'UNKNOWN',
    businessRoleSource: raw.businessRoleSource ?? raw.business_role_source ?? 'NONE',
    businessRoleIsManual: raw.businessRoleIsManual ?? raw.business_role_is_manual ?? false,
    businessRoleNote: raw.businessRoleNote ?? raw.business_role?.note ?? null,
    businessRoleDecidedAt: raw.businessRoleDecidedAt ?? raw.business_role?.decided_at ?? null,
    businessRoleConflict: raw.businessRoleConflict ?? false,
    businessRoleSignals: safeArray(raw.businessRoleSignals).map(signal => ({
      ...signal,
      casNumber: signal.casNumber ?? signal.cas_number,
      productName: signal.productName ?? signal.product_name,
      selfDeclared: signal.selfDeclared ?? signal.self_declared ?? false,
