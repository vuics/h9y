...
export default function OverviewPage() {
  ...
  return <div className="pr-stack pr-stack--lg">
    ...
    <section><div className="pr-section-heading"><div><h2>Активные закупки по этапам</h2><p>Карточки сгруппированы по фактическому состоянию процесса.</p></div><Link to="/procurement/requests">Все карточки</Link></div>
      <div className="pr-stage-board">{(data.stages || []).map(stage => <div className="pr-stage" key={stage.id}><div className="pr-stage__header"><span>{stage.label}</span><Badge>{stage.count}</Badge></div><div className="pr-stage__cards">{stage.cards?.map(card => <button key={card.id} onClick={() => navigate(`/procurement/requests/${card.id}`)} className="pr-case-card"><div><strong>{card.title}</strong><span>CAS {card.casNumber || 'не указан'} · {card.targetVolume || 'объём не указан'}</span></div><StatusBadge status={card.completeness || card.stage} compact /></button>)}{!stage.cards?.length && <div className="pr-stage__empty">Нет карточек</div>}</div></div>)}</div>
    </section>
    <div className="pr-overview-grid"><Card><CardHeader><CardTitle>Требует внимания</CardTitle><Link to="/procurement/escalations">Вся очередь</Link></CardHeader><CardContent className="pr-attention-list">{data.attention?.length ? data.attention.slice(0, 4).map(item => <Link to={`/procurement/escalations?selected=${item.id}`} key={item.id} className="pr-attention"><div className="pr-attention__priority">{item.priority}</div><div><strong>{item.title}</strong><span>{item.cardTitle} · {item.supplierName}</span></div><StatusBadge status={item.status} compact /></Link>) : <EmptyState title="Очередь пуста" description="Открытых эскалаций нет." />}</CardContent></Card>
      <Card><CardHeader><CardTitle>Последние изменения</CardTitle><Link to="/procurement/activity">Журнал</Link></CardHeader><CardContent className="pr-activity-list">{data.recentActivity?.map(item => <div className="pr-activity" key={item.id}><div className={`pr-activity__dot pr-activity__dot--${item.level}`} /><div><strong>{item.title}</strong><p>{item.description}</p><time>{new Date(item.createdAt).toLocaleString('ru-RU')}</time></div></div>)}</CardContent></Card></div>
    {data.kpis?.failures > 0 && <Alert tone="warning" icon={<AlertTriangle />} title="Есть необработанные сбои"><p>Ошибки интеграций и вложений показаны отдельно от бизнес-статусов и не скрываются автоматическими повторами.</p><Link to="/procurement/activity?level=error">Открыть ошибки</Link></Alert>}
  </div>
}
