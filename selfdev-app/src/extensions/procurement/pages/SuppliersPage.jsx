...
export default function SuppliersPage() {
  ...
  return <div className="pr-stack"><div className="pr-section-heading"><div><h2>Поставщики и контакты</h2><p>Квалификация, источники подтверждения, вещества и история коммуникаций.</p></div></div><ListFilters filters={filters} onChange={setFilters} statuses={[{ value: 'UNVERIFIED', label: 'Не проверен' }, { value: 'UNDER_REVIEW', label: 'На проверке' }, { value: 'QUALIFIED', label: 'Квалифицирован' }, { value: 'REJECTED', label: 'Отклонён' }]} placeholder="Компания, страна или контакт" />
    {query.isLoading ? <LoadingState /> : query.isError ? <ErrorState error={query.error} onRetry={query.refetch} /> : <><DataTable rows={query.data.items} onRowClick={row => navigate(`/procurement/suppliers/${row.id}`)} emptyTitle="Поставщиков пока нет" columns={[
      { id: 'name', header: 'Поставщик', cell: row => <div className="pr-primary-cell"><strong>{row.name}</strong><span>{row.id}</span></div> },
