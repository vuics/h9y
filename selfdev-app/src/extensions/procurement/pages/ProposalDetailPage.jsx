...
export default function ProposalDetailPage() {
  ...
  return <DetailLayout backTo="/procurement/proposals" backLabel="Все предложения" eyebrow={`${proposal.id} · ревизия ${proposal.revision}`} title={proposal.supplierName} status={<StatusBadge status={proposal.completeness} />} meta={<><SupplierLink supplierId={proposal.supplierId} name={proposal.supplierName} /> · <Link to={`/procurement/requests/${proposal.cardId}`}>Карточка #{proposal.cardId}</Link> · Идентичность: <StatusBadge status={proposal.productIdentityStatus === 'UNVERIFIED' ? 'UNVERIFIED_IDENTITY' : proposal.productIdentityStatus} compact /></>} actions={actions} warnings={<>{proposal.warnings?.length > 0 && <Alert><AlertTriangle /><AlertTitle>Предложение нельзя считать полностью готовым</AlertTitle><AlertDescription><ul>{proposal.warnings.map(warning => <li key={warning}>{warning}</li>)}</ul></AlertDescription></Alert>}{clarify.isError && <Alert><AlertTriangle /><AlertTitle>Уточнение не подготовлено</AlertTitle><AlertDescription>{clarificationError}</AlertDescription></Alert>}</>}>
    <Card><CardHeader><CardTitle>Коммерческие условия</CardTitle><CardAction><Link to={`/procurement/proposals/compare?cardId=${proposal.cardId}`}>Сравнить по карточке</Link></CardAction></CardHeader><CardContent><div className="pr-field-grid">{fields.map(([key, label, format]) => { const value = format(proposal); const state = proposal.fieldStates?.[key] || (value ? 'PRESENT' : 'UNKNOWN'); const quote = proposal.evidence?.[key]?.[0]?.quote; return <div className={`pr-field pr-field--${state.toLowerCase()}`} key={key}><div><span>{label}</span><StatusBadge status={state} compact /></div><strong>{value || 'Не указано'}</strong>{proposal.originalValues?.[key] && <small>Исходное значение: {proposal.originalValues[key]}</small>}{/* The sentence the value was read from. A field with a number and no quote behind it is a claim; with the quote it is evidence. */}{quote && <blockquote className="pr-evidence">«{quote}»</blockquote>}</div> })}</div></CardContent></Card>
    ...
    {canReadCommunications && <Card><CardHeader><CardTitle>Уточнение поставщику</CardTitle><CardAction><div className="pr-inline-actions"><Button variant="outline" size="sm" isDisabled={clarify.isPending} onPress={() => clarify.mutate('ru')}>Подготовить RU</Button><Button variant="outline" size="sm" isDisabled={clarify.isPending} onPress={() => clarify.mutate('en')}>Подготовить EN</Button></div></CardAction></CardHeader><CardContent>{clarification ? <><p className="pr-note">Черновик построен сервером только по сохранённым пробелам. Он не сохранён и не отправлен.</p><Textarea value={clarification.draft} readOnly rows={12} /><div className="pr-inline-actions"><Button size="sm" onPress={() => navigator.clipboard.writeText(clarification.draft)}>Копировать</Button><StatusBadge status={clarification.language.toUpperCase()} compact /></div></> : <p className="pr-note">Подготовьте RU- или EN-черновик. Отправка остаётся отдельным действием в существующем чате/канале переговоров.</p>}</CardContent></Card>}
    <Card><CardHeader><CardTitle>Источники</CardTitle><p className="pr-note">Текст, из которого извлечены значения выше. Хеш подтверждает, что он не менялся; сам текст отвечает на вопрос, ради которого источник и открывают.</p></CardHeader><CardContent className="pr-source-references">{proposal.sourceReferences?.map(source => <div key={source.sourceId}><div><strong>{source.sourceId}</strong><span>{source.channel} · {source.receivedAt ? new Date(source.receivedAt).toLocaleString('ru-RU') : '—'}</span></div><code>{source.sha256}</code>{source.negotiationId && <Link to={`/procurement/negotiations/${source.negotiationId}`}>Открыть в переговорах</Link>}{source.text && <details className="pr-source-text"><summary>Показать текст источника</summary><pre className="pr-message-text">{source.text}{source.truncated ? '\n…' : ''}</pre></details>}</div>)}</CardContent></Card>
  </DetailLayout>
}
