    ...
    <div className="pr-stack">
      ...
      {run && <>
        ...
        <div className="pr-sourcing-workbench">
          ...
          {candidate && <div className="pr-sourcing-detail">
            ...
            <Card><CardHeader><CardTitle>Решение специалиста</CardTitle><StatusBadge status={candidate.reviewDecision} /></CardHeader><CardContent>
              {!canReviewSourcing ? <p className="pr-note">Для квалификации требуется разрешение SOURCING_REVIEW.</p> : <form className="pr-sourcing-review" onSubmit={event => { event.preventDefault(); if (reviewNote.trim()) review.mutate() }}><label className="pr-form-field"><span>Решение</span><Select selectedKey={reviewDecision} onSelectionChange={setReviewDecision}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{reviewOptions.map(([value, label]) => <SelectItem key={value} id={value}>{label}</SelectItem>)}</SelectContent></Select></label><label className="pr-form-field"><span>Обоснование <b>*</b></span><Textarea value={reviewNote} onChange={event => setReviewNote(event.target.value)} placeholder="Укажите, какие источники проверены и почему принято это решение." required /></label><Button type="submit" isDisabled={!reviewNote.trim() || review.isPending}>{review.isPending ? 'Сохранение…' : 'Сохранить решение'}</Button></form>}
        ...
        <Card><CardHeader><div><CardTitle>Источники поиска</CardTitle><p>Сохранённые URL и время получения обеспечивают трассируемость оценки.</p></div><span>{run.sources?.length || 0}</span></CardHeader><CardContent>
          {canResearchSourcing && <form className="pr-sourcing-add-source" onSubmit={event => { event.preventDefault(); if (sourceUrl.trim()) addSource.mutate() }}><label className="pr-form-field"><span>Добавить официальный документ или страницу</span><Input type="url" value={sourceUrl} onChange={event => setSourceUrl(event.target.value)} placeholder="https://company.example/permits/..." required /></label><Button type="submit" variant="outline" isDisabled={!sourceUrl.trim() || addSource.isPending}>{addSource.isPending ? 'Обработка…' : 'Добавить и проанализировать'}</Button></form>}
          {!run.sources?.length ? <EmptyState title="Источники не сохранены" /> : <div className="pr-sourcing-sources">{run.sources.map(source => <article key={source.id}><div><StatusBadge status={source.sourceType} compact /><strong>{source.title || source.domain}</strong><span>{source.query || 'Добавлен вручную'}</span></div><div><span>{source.claimCount || 0} утверждений · {source.fetchStatus}</span><SourceLink source={source} /></div>{source.extractionWarnings?.length > 0 && <small>{source.extractionWarnings.join(' · ')}</small>}</article>)}</div>}
