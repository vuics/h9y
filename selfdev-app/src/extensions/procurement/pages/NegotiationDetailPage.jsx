...
export default function NegotiationDetailPage() {
  ...
  return <DetailLayout backTo="/procurement/negotiations" backLabel="Все переговоры" eyebrow={negotiation.id} title={negotiation.supplierName} status={<StatusBadge status={negotiation.status} />} meta={<><Link to={`/procurement/requests/${negotiation.cardId}`}>{negotiation.cardTitle}</Link> · {negotiation.contactName || negotiation.contactId}</>} actions={<RouterLinkButton to={`/chat?context=procurement-negotiation:${negotiation.id}`}><MessageSquare size={16} />Открыть разговор с агентом</RouterLinkButton>} warnings={(negotiation.requiresHuman || negotiation.lastWorkerError) && <Alert><AlertTriangle /><AlertTitle>Требуется внимание специалиста</AlertTitle><AlertDescription>{negotiation.lastWorkerError || 'Переговоры приостановлены до решения открытой эскалации.'}</AlertDescription></Alert>}>
