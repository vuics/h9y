...
export default function ProposalsPage() {
  ...
  return <div className="pr-stack"><div className="pr-section-heading"><div><h2>Предложения</h2><p>Нормализованные коммерческие условия с исходными значениями и признаками качества.</p></div>{filters.cardId && <Button asChild><Link to={`/procurement/proposals/compare?cardId=${filters.cardId}`}>Сравнить предложения</Link></Button>}</div><ListFilters filters={filters} onChange={setFilters} statuses={[{ value: 'COMPLETE', label: 'Готово' }, { value: 'NEEDS_CLARIFICATION', label: 'Нужно уточнение' }, { value: 'CONFLICTING', label: 'Противоречия' }, { value: 'NEEDS_HUMAN_REVIEW', label: 'Нужен специалист' }]} placeholder="Поставщик, валюта, Incoterm или RESP-ID" />
