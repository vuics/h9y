...
export default function RequestDetailPage() {
  ...
  return <DetailLayout backTo="/procurement/requests" backLabel="Все карточки" eyebrow={`Карточка #${card.id}`} title={card.title} status={<StatusBadge status={card.stage} />} meta={<>Обновлено {card.updatedAt ? new Date(card.updatedAt).toLocaleString('ru-RU') : '—'}</>} actions={<Button asChild><Link to={`/chat?context=procurement-card:${card.id}`}><MessageSquare size={16} />Спросить агента</Link></Button>} warnings={warning && <Alert tone="warning" icon={<AlertTriangle />} title="Данные требуют проверки"><p>Нормализация или полнота карточки не позволяют считать все параметры подтверждёнными.</p></Alert>}>
