...
const resources = {
  'en-US': {
    ...
    Vault: {
      ...
      'Your keys are safely protected...': 'Your keys are safely protected and securely stored. We use advanced encryption methods to ensure that your keys are never exposed in plain text. They are encrypted before storage and kept protected by industry-leading security protocols, so only you and authorized systems have access. Your privacy and security are our top priorities.',
      'Insecure Secret Storage': 'Insecure Secret Storage',
      'Your keys are stored in your local database...': 'Your keys are stored in your local database for simplicity and fast setup. This mode prioritizes ease of use over advanced security features. Secrets are not managed by an external vault system and therefore rely on application-level access control. For production and sensitive workloads, we strongly recommend using the Cluster version with Vault integration, which provides hardened encryption, auditability, and secure secret isolation. Upgrade to Cluster mode for production-grade security.',
    },
    Settings: {
      settings: 'Settings',
      language: 'Language',
  ...
  'ru-RU': {
    ...
    Vault: {
      ...
      'Cancel': 'Отмена',
      'Submit': 'Отправить',
      'Secure Secret Storage': 'Надежное хранилище секретов',
      'Your keys are safely protected...': 'Ваши ключи надежно защищены и безопасно хранятся. Мы используем передовые методы шифрования, чтобы гарантировать, что ваши ключи никогда не будут храниться в открытом виде. Они шифруются перед сохранением и защищаются с помощью лучших отраслевых протоколов безопасности, поэтому доступ имеют только вы и авторизованные системы. Ваша конфиденциальность и безопасность — наш главный приоритет.',
      'Insecure Secret Storage': 'Небезопасное хранилище секретов',
      'Your keys are stored in your local database...': 'Ваши ключи хранятся в локальной базе данных для простоты и быстрой настройки. Этот режим ориентирован на удобство использования, а не на расширенные функции безопасности. Секретные данные не управляются внешней системой управления секретами и, соответственно, зависят от контроля доступа на уровне приложения. Для использования в рабочей среде и при работе с чувствительными данными настоятельно рекомендуется использовать кластерную версию с интеграцией системы управления секретами, которая обеспечивает усиленное шифрование, аудит действий и изолированное хранение секретных данных. Перейдите на кластерный режим для обеспечения безопасности уровня промышленной эксплуатации.',
    },
    Settings: {
      settings: 'Настройки',
      language: 'Язык',
